...
research_task = Task(
    ...
    """),
    expected_output="A comprehensive 1 paragraph short report on the latest information on the given topic: {topic}.",
...
write_task = Task(
    ...
    """),
    expected_output="A 1 paragraph short article on the topic: {topic}, formatted as markdown.",
    tools=[google_search_tool],
    agent=writer,
...
proof_read_task = Task(
    ...
    Also write 1 source for further studying of the topic. This article should be digestible, easy to understand, engaging and positive.
    """),
    expected_output="A 2 paragraph long article on the topic: {topic}, formatted as markdown with all the relevant sources",
    tools=[google_search_tool],
    agent=proof_reader,
    async_execution=False,
